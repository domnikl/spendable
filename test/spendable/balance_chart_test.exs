defmodule Spendable.BalanceChartTest do
  ...
  describe "balance chart functionality" do
    setup do
      ...
      # 1500.00 EUR
      {:ok, _} = Accounts.upsert_account_balance(account, yesterday, 150_000, "EUR")
      # 2000.00 EUR
      {:ok, _} = Accounts.upsert_account_balance(account, last_week, 200_000, "EUR")
      # 1750.00 EUR
      {:ok, _} = Accounts.upsert_account_balance(account, today, 175_000, "EUR")
    ...
    test "chart data conversion handles cents to euros properly", %{account: account} do
      ...
      # 1234.56 EUR
      {:ok, _} = Accounts.upsert_account_balance(account, test_date, 123_456, "EUR")
      ...
      # Float.round(123456 / 100.0)
      assert day_amount == 1235.0
    end
  end
  ...
  describe "user preferences" do
    test "can update preferred chart account" do
      ...
      {:ok, updated_user} =
        Users.update_user_preferences(user, %{preferred_chart_account_id: account.id})
