defmodule Spendable.PartialFinalizationTest do
  ...
  describe "partial transaction finalization" do
    ...
    test "creating payment updates finalized amount", %{user: user, transaction: transaction, budget: budget} do
      ...
      assert updated_transaction.finalized == false  # Not fully finalized yet
      assert Transactions.get_remaining_amount(updated_transaction) == 4000
    end

    test "creating second payment that completes transaction", %{user: user, transaction: transaction, budget: budget} do
      ...
      {:ok, _payment1} = Payments.create_payment_from_transaction(user, transaction, payment_attrs_1)
      ...
      {:ok, _payment2} = Payments.create_payment_from_transaction(user, updated_transaction, payment_attrs_2)
    ...
    test "prevents payment that exceeds remaining amount", %{user: user, transaction: transaction, budget: budget} do
      # Create first payment for 6000 cents
      payment_attrs_1 = %{
        "amount" => 6000,
        "budget_id" => budget.id,
    ...
    test "unfinalized transactions query includes partially finalized transactions", %{user: user, transaction: transaction, budget: budget} do
