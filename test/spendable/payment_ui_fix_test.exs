defmodule Spendable.PaymentUIFixTest do
  ...
  describe "payment form data type handling" do
    setup do
      ...
      # Create account
      account = %Spendable.Accounts.Account{
      ...
      # Create budget
      budget = %Spendable.Budgets.Budget{
        name: "Test Budget",
      ...
      transaction = %Transaction{
        ...
        amount: -10000,  # Negative amount (expense)
        currency: "EUR", 
    ...
    test "handles string payment amounts from form", %{user: user, transaction: transaction, budget: budget} do
      # Simulate form data (strings from HTML forms)
      payment_attrs = %{
        "amount" => "6000",  # String from HTML form
      ...
      assert {:ok, _payment} = Payments.create_payment_from_transaction(user, transaction, payment_attrs)
    end

    test "handles integer payment amounts", %{user: user, transaction: transaction, budget: budget} do
    ...
    test "correctly calculates remaining amount for negative transactions", %{transaction: transaction} do
    ...
    test "would_exceed_remaining_amount works with negative transactions", %{transaction: transaction} do
      # Should not exceed for valid amounts
      refute Transactions.would_exceed_remaining_amount?(transaction, "5000")  # String
      refute Transactions.would_exceed_remaining_amount?(transaction, 5000)   # Integer
      refute Transactions.would_exceed_remaining_amount?(transaction, "10000") # String exact
      
      # Should exceed for amounts larger than remaining
      assert Transactions.would_exceed_remaining_amount?(transaction, "15000") # String
      assert Transactions.would_exceed_remaining_amount?(transaction, 15000)  # Integer
    end

    test "query for unfinalized transactions includes negative amounts", %{user: user, transaction: transaction} do
